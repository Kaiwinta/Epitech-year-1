...
static int check_value(char comp, char value, int *enc)
{
    if (value == comp) {
            if (*enc)
                return ERROR;
...
static int is_valid_boat(char *line, int nb)
{
    ...
    if (check_value('2', line[0], &enc_2) != 0)
        return ERROR;
    if (check_value('3', line[0], &enc_3) != 0)
        return ERROR;
    if (check_value('4', line[0], &enc_4) != 0)
        return ERROR;
    if (check_value('5', line[0], &enc_5) != 0)
        return ERROR;
    return SUCCESS;
}
